...
def configure(repl):
    ...
    # PK - Add custom keybinding for accepting auto-suggestions; use control-o as I've done for zsh 
    # and vim
    @repl.add_key_binding('c-o', filter=ViInsertMode())  # Replace with your desired key combo
    def _(event):
        """
        Accept auto-suggestions when in vi insert mode or other context.
        """
        buff = event.current_buffer
        if buff.suggestion:  # Check if there is a suggestion
            buff.insert_text(buff.suggestion.text)  # Insert the suggestion text
